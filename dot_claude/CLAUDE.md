...
- Before researching a topic, check memory files and project docs for prior research.
- When saving research, always include the date (YYYY-MM-DD) so staleness is visible.
- Save research findings to persistent storage (auto-memory or project `docs/research/`).
- Reuse existing research when still current. Don't re-research what's already known.
